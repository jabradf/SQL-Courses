...
-- Your Script Here 
select 
    SPLIT_PART(TRIM(PRODUCT,' 0'),',',1) AS Prod_name, 
    SPLIT_PART(TRIM(PRODUCT,' 0'),',',2) AS Prod_colour,
    TRIM(ordercity) AS order_city,
    TO_DATE(orderdate,'MMMM DD, YYYY') AS order_date,
    orderid, quantity
FROM orders;
...
FROM orders
WHERE TO_DATE(orderdate,'MMMM DD, YYYY')  between '2023-01-01' and '2023-01-31'
	GROUP BY Prod_name, order_city
	ORDER BY  TOTAL_QUANTITY_SOLD DESC
	LIMIT 5;
